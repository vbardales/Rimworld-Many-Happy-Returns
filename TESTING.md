...
| Existing save, log audited | `Submit-PickleRun.ps1 -Mod ManyHappyReturns ... -DepMap wsl-deps.avec-loadaudit.map -Language English -Filter 16-existing-save-regression.feature`, then `-Language French` | S16, the existing-save half: the mod in the saved colony from the letter to the verdict, both settings routes, then the game log read for this mod (LoadAudit). The new-colony half is played once, in the final pass, in its own feature. |
| New colony, once | `Submit-PickleRun.ps1 -Mod ManyHappyReturns ... -DepMap wsl-deps.avec-newcolony.map -Language English -Filter 17-new-colony -Extra "-pickle-scenario-timeout=400"` | S16's new-colony half: a colony started from the main menu, a birthday from the letter to the verdict, the log read. Played once, in the final pass: a new colony is not reproducible. |
